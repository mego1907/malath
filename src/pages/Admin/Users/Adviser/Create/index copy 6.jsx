...
const AdviserUsersEdit = () => {
  ...
  // console.log("🚀 ~ file: index.jsx:47 ~ AdviserUsersEdit ~ dataCountries:", dataCountries)
  ...
  const { dataNationality, loading:loadingNationality } = useSelector((state) => state.nationality);
  const { dataLanguages, loading:loadingLanguages } = useSelector((state) => state.languages);
  ...
  const onFinish = (values) => {
    console.log("🚀 ~ file: index.jsx:89 ~ onFinish ~ values:", values)
    let reqObj = {};
  ...
  const handleChange2 = ({ file }) => {
    if (file && file?.originFileObj) {
      ...
      console.log("🚀 ~ file: index.jsx:191 ~ handleChange2 ~ blobURL:", blobURL)
    }
  };
  ...
  useEffect(() => {
    if (isUpdate) {
      ...
      const subfieldIds2 = [];
      dataUser?.languages?.forEach((obj) => {
        subfieldIds2.push(obj.id);
      });
      ...
      const { fields, video , avatar, ...otherData } = dataUser;
      
      const getCountry = dataCountries?.data?.find(
        (item) => item?.id === dataUser?.country?.id
      ); 
      // console.log("🚀 ~ file: index.jsx:211 ~ useEffect ~ getCountry:", getCountry)
      // const getNationalites = dataNationality?.data?.find(
      //   (item) => item.name === dataUser?.nationalites
      // );
      // console.log(dataUser?.languages)
      // const getLanguages = dataLanguages?.data?.find(
      //   (item) => item.name === dataUser?.languages
      // );
      ...
      form.setFieldsValue({
        ...
        languages: subfieldIds2,
        nationality: dataUser?.nationality?.id,
        city: Number(getCities?.id),
        fields: subfieldIds,
      });
      setImageUrl(avatar);
    }
  }, [form, dataUser , dataCountries]);
      ...
      {loadingUser ? (
        ...
        >
          <Row gutter={20}>
            ...
            <Col span={24} md={24}>
              <Form.Item name="video" className="text-center">
                <Upload
                  beforeUpload={false}
                  name="video"
                  // showUploadList={false}
            ...
            <Col span={24} md={12}>
              ...
              >
                ...
                >
                  {dataNationality?.data?.map((item, index) => (
            ...
            <Col span={24} md={12}>
              ...
              >
                ...
                >
                  {dataLanguages?.data?.map((item, index) => (
                    <Option value={item.id}>{item.name}</Option>
                  ))}
                </Select>
              </Form.Item>
            ...
            <Col span={24} md={12}>
              <Form.Item
                name="about"
              >
                <Input.TextArea  rows={4} placeholder="نبذة عن المستشار باللغة العربية" />
              </Form.Item>
            </Col>
            <Col span={24} md={12}>
              <Form.Item
                name="about_ar"
              >
                <Input.TextArea rows={4} placeholder="نبذة عن المستشار باللغة الانجليزية" />
              </Form.Item>
            </Col>
          </Row>
