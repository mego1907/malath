...
const AdviserUsersEdit = () => {
  ...
  console.log("🚀 ~ file: index.jsx:62 ~ AdviserUsersEdit ~ dataLanguages:", dataLanguages)
  ...
  const onFinish = (values) => {
    ...
    const getNationalites = dataNationality?.data?.find(
      (item) => item.name === dataUser?.nationality
      );
      // console.log(dataLanguages)

      // const getLanguages = dataLanguages?.data?.find(
      //   (item) => item.name === dataUser?.languages.map(())
      // );
    ...
    values = {
      ...
      timezone: "+3",
      id: id,
      nationality:getNationalites?.id,
      // city: CityId,
      role: "ADVISER",
  ...
  useEffect(() => {
    if (isUpdate) {
      ...
      const getCountry = dataCountries?.data?.find(
        (item) => item.name_ar === dataUser?.country
      );
      // const getNationalites = dataNationality?.data?.find(
      ...
      form.setFieldsValue({
        ...
        // nationalites: getNationalites?.id,
